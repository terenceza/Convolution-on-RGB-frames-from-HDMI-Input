...
#include "xil_cache.h"
#include "xaccelconvfilter.h"
#include "xaccelconvolute1.h"
...
XAccelconvolute1 Inst;
XAccelconvfilter Inst1;

void ACSRGB2GrayInitialize()
{
    if (XAccelconvolute1_Initialize(&Inst, 0) != XST_SUCCESS )
    {
    	xil_printf("Failed Initializing XAccelconvolute1");
    }

    XAccelconvolute1_EnableAutoRestart(&Inst);
    XAccelconvolute1_Set_CHANNEL_ID(&Inst, 0);
    XAccelconvolute1_Start(&Inst);

    XAccelconvolute1_EnableAutoRestart(&Inst);

    int isReady = XAccelconvolute1_IsReady(&Inst);
    xil_printf("XAccelconvolute1 is ready = %d \n", isReady);
}
...
//							  2, 0, -2 ,
//							  1, 0, -1 };
#define DDR_BASE_ADDR_IN  0x0000000000000064

void ACSAccelConvFilterInitialize()
{
    if (XAccelconvfilter_Initialize(&Inst1, 0) != XST_SUCCESS )
    {
    	xil_printf("Failed Initializing XAccelconvfilter");
    }

    XAccelconvfilter_Set_PassThroughP(&Inst1, 0);
    printf("GetPassThrough = %d \n", XAccelconvfilter_Get_PassThroughP(&Inst1));
    XAccelconvfilter_Set_MatRowsP(&Inst1, 480);
    XAccelconvfilter_Set_MatRowsP(&Inst1, 640);
	
	XAccelconvfilter_Set_FilterDimX(&Inst1, 3);
	XAccelconvfilter_Set_FilterDimY(&Inst1, 3);

    XAccelconvfilter_EnableAutoRestart(&Inst1);

	XAccelconvfilter_WriteReg(DDR_BASE_ADDR_IN, 0,  1);
	XAccelconvfilter_WriteReg(DDR_BASE_ADDR_IN, 4,  0);
	XAccelconvfilter_WriteReg(DDR_BASE_ADDR_IN, 8, -1);
	XAccelconvfilter_WriteReg(DDR_BASE_ADDR_IN, 12, 2);
	XAccelconvfilter_WriteReg(DDR_BASE_ADDR_IN, 16, 0);
	XAccelconvfilter_WriteReg(DDR_BASE_ADDR_IN, 20,-2);
	XAccelconvfilter_WriteReg(DDR_BASE_ADDR_IN, 24, 1);
	XAccelconvfilter_WriteReg(DDR_BASE_ADDR_IN, 28, 0);	
	XAccelconvfilter_WriteReg(DDR_BASE_ADDR_IN, 32,-1);

	XAccelconvfilter_Set_FilterInData(&Inst1, DDR_BASE_ADDR_IN);
    

	int x0 = XAccelconvfilter_ReadReg(DDR_BASE_ADDR_IN, 0);
	int x12 = XAccelconvfilter_ReadReg(DDR_BASE_ADDR_IN, 12);

	u64 fdata = XAccelconvfilter_Get_FilterInData(&Inst1);
	printf("x0 = %d, x12 = %d fdata=%x %x", x0, x12, fdata & 0xFFFFFFFF00000000 >> 32, fdata & 0xFFFFFFFF);

	XAccelconvfilter_Start(&Inst1);

    XAccelconvfilter_EnableAutoRestart(&Inst1);

    int isReady = XAccelconvfilter_IsReady(&Inst1);
    xil_printf("XAccelconvfilter is ready = %d \n", isReady);
}
...
int main(void)
{
	ACSAccelConvFilterInitialize();
	ACSRGB2GrayInitialize();
...
void DemoPrintMenu()
{
	...
	xil_printf("9 - Flip Filter Convolution Paaa Through\n\r");
	xil_printf("q - Quit\n\r");
	xil_printf("\n\r");
	xil_printf("\n\r");
...
void DemoChangeRes()
{
	...
	while (!fResSet)
	{
		...
		case'9':
		    if (XAccelconvfilter_Get_PassThroughP(&Inst1) == 0)
			{
		    	XAccelconvfilter_Set_PassThroughP(&Inst1, 1);
			}
			else
			{
				XAccelconvfilter_Set_PassThroughP(&Inst1, 0);
